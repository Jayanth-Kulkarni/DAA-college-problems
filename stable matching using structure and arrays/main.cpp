...
{
	cout << "Enter the preference of the men starting from 1 to "<<MAX+1<<"!"<< endl;
	FOR(i, MAX)
	{
		...
		FOR(j, MAX)
		{
		    ...
		    cin>>mem;
		    mem--;
			m.preference[i][j]=mem;

		}
	}

	cout << "Enter the preference of the women starting from 1 to "<<MAX+1<<"!"<< endl;
	FOR(i, MAX)
	{
...
void perform_match(match &m, match &w)
{
	...
	while ((i<MAX) && m.flag[i] == 0||(overall_flag<MAX))
	{
